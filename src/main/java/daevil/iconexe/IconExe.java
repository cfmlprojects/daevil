...
public class IconExe {
     ...
     */
    static List<IconResInfo> unloadIcons(String program, ImageData[] icons) throws IOException {
    ...
    public static void setIcon(File exe, String resourcePath) throws Exception {
        if (!exe.exists()) throw new AssertionError("EXE file does not exist! " + exe.getAbsolutePath());
        InputStream stream = Daevil.class.getResourceAsStream(resourcePath);
        ImageLoader loader = new ImageLoader();
        List<ImageData> images = new ArrayList<>();
        if(stream == null){
            if(Files.exists(Paths.get(resourcePath))){
                try(InputStream streama = new FileInputStream(Paths.get(resourcePath).toFile())){
                    images = getImageData(exe, resourcePath, loader, images, streama);
                } catch (Exception e){
                    e.printStackTrace();
                }
            }
        } else {
            images = getImageData(exe, resourcePath, loader, images, stream);
        }
        ImageData[] data = new ImageData[images.size()];
        data = images.toArray(data);

        List<IconResInfo> nMissing = unloadIcons(exe.getAbsolutePath(), data);

        if (nMissing.size() != 0)
            System.err.println("Error - " + nMissing.size()
                    + " icon(s) not replaced in " + exe.getAbsolutePath()
                    + " using " + resourcePath);

    }

    private static List<ImageData> getImageData(File exe, String resourcePath, ImageLoader loader, List<ImageData> images, InputStream streama) throws IOException {
        try {
            if (resourcePath.toLowerCase().endsWith(".png")) {
                images = exeIconsFromPng(exe.getAbsolutePath(), streama);
            } else {
                // An ICO should contain 7 images, a BMP will contain 1
                ImageData[] current = loader.load(streama);
                for (int j = 0; j < current.length; j++) {
                    System.out.println("loaded an image " + j);
                    images.add(current[j]);
                }
            }
        } catch (RuntimeException e) {
    ...
    public static void setIcon(File exe, File ico) throws Exception {
        ImageLoader loader = new ImageLoader();
        if (!ico.exists()) throw new AssertionError("Image file does not exist! " + ico.getAbsolutePath());
        if (!exe.exists()) throw new AssertionError("EXE file does not exist! " + exe.getAbsolutePath());

        List<ImageData> images = new ArrayList<>();
        try {
            if (ico.getName().toLowerCase().endsWith(".png")) {
                images = exeIconsFromPng(exe.getAbsolutePath(), ico);
            } else {
                // An ICO should contain 7 images, a BMP will contain 1
                ImageData[] current = loader.load(ico.getAbsolutePath());
                for (int j = 0; j < current.length; j++) {
                    System.out.println("loaded an image " + j);
                    images.add(current[j]);
                }
    ...
    public static List<ImageData> exeIconsFromPng(String program, Object imgFile) throws IOException {
        ...
        if (imgFile instanceof File) {
            image = ImageIO.read((File) imgFile);
        } else if (imgFile instanceof InputStream) {
            image = ImageIO.read((InputStream) imgFile);
        } else {
            throw new IllegalArgumentException("Unknown type of thing passed");
        }
        ImageLoader loader = new ImageLoader();
        List<ImageData> images = new ArrayList<ImageData>();
        for (int i = 0; i < iconInfo.length; i++) {
            int width = iconInfo[i].data.width;
            int height = iconInfo[i].data.height;
            int depth = iconInfo[i].data.depth;
            try {
                Daevil.log.trace(String.format("Scaling image to requested dimensions - width: %s   height: %s   depth: %s", width, height, depth));
                BufferedImage newImage = getBuff(iconInfo[i].data);
                newImage = Depth.convert(newImage, depth);
                Image thumbnail = image.getScaledInstance(width, height, Image.SCALE_SMOOTH);
                newImage.getGraphics().drawImage(thumbnail, 0, 0, Color.white, null);
//				String tmpBmp = "/tmp/bmp"+ i +"_"+ width+"x"+height+"x"+depth+".bmp";
//				out = new FileOutputStream(tmpBmp);
                ByteArrayOutputStream out = new ByteArrayOutputStream();
                if (depth == 32) {
                    WriteBMP32.write(newImage, out);
                } else {
                    ImageIO.write(newImage, "bmp", out);
                }
                ImageData[] current = loader.load(new ByteArrayInputStream(out.toByteArray()));
                for (int j = 0; j < current.length; j++) {
                    images.add(current[j]);
                }
            } catch (Exception e) {
    ...
    public static BufferedImage getBuff(ImageData data) {
    ...
    static class ImageData {
         ...
         * @throws IndexOutOfBoundsException if getWidth is too large
         * @throws IllegalArgumentException  <ul>
		...
		*/
         ...
         * @throws IndexOutOfBoundsException if putWidth is too large
         * @throws IllegalArgumentException  <ul>
         *                                   <li>ERROR_NULL_ARGUMENT - if pixels is null</li>
         *                                   <li>ERROR_INVALID_ARGUMENT - if x or y is out of bounds</li>
    ...
    static class LEDataInputStream extends InputStream {
         ...
         * @throws java.io.IOException if the pushback buffer is too small
         */
